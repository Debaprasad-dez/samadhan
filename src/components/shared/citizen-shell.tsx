...
export function CitizenShell({
  ...
  // Home, cases and profile ship the design's own full-bleed layout (top bar,
  // hero, bottom nav), so the shell steps aside on those routes.
  if (pathname === "/" || pathname === "/cases" || pathname === "/profile")
    return <>{children}</>;
